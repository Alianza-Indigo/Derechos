...
export async function POST(request: NextRequest) {
  ...
  // Resuelve el tenant del usuario para acotar prompts y proveedores.
  ...
  const [dbPrompt] = body.promptId
    ? await db.select().from(schema.aiPromptTemplates).where(and(eq(schema.aiPromptTemplates.id, body.promptId), eq(schema.aiPromptTemplates.organizationId, user.organizationId))).limit(1)
    : await db.select().from(schema.aiPromptTemplates).where(eq(schema.aiPromptTemplates.organizationId, user.organizationId)).limit(1);
  if (!dbPrompt) {
